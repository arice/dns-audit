...
Tool that queries DNS records for all customer domains and writes one Markdown file per domain into `./dns_records/`. Optionally syncs each file to 1Password as a Secure Note via the `op` CLI.
...
# Archive previous output before overwriting, then sync
python3 dns_fetch.py --archive --op-sync
```
...
Multiple rows for the same customer are fine (e.g. a client with both a `.com` and `.org` domain, or a client with multiple different domains).
